...
//   // 🔄 FETCH DATA FROM MONGODB + DEDUPLICATE REPLACEMENT FILTER
...
//         // Ensure no duplicated line entries ever break layout
//         const uniqueMap = new Map();
//         rawData.forEach((item: any) => {
//           const uniqueId = item._id || item.id;
//           // By tracking via map, any duplicate record match replaces previous position array placement completely
//           uniqueMap.set(uniqueId, item);
//         });

//         setMovements(Array.from(uniqueMap.values()));
//       } else {
...
//     // Optimistic direct inline row values replacement setup
//     setMovements((prev) =>
//       prev.map((m) => {
...
//       if (response.success) {
//         setEditingId(null);
//         await loadData(); // Safe deduplicated remote load sync
//       } else {
...
//     ) {
//       setMovements(movements.filter((m) => m._id !== id && m.id !== id));
//     }
//   };
...
//                       /* ✏️ INLINE EDIT MODE (REPLACES TEXT ON THE EXACT SAME LINE ROW) */
//                       <tr
//                         key={rowId}
...
//                             <button
//                               onClick={() => handleInlineSave(rowId)}
...
//                             </button>
//                             <button
//                               onClick={() => handleDeleteMovement(rowId)}
...
//       {/* COMPACT DIALOG OVERLAY SHEET FOR NEW CREATIONS ONLY */}
//       {isModalOpen && (
...
export default function MovementPage() {
  ...
  async function loadData() {
    try {
      const response = await getMovements();
      if (response.success && response.data) {
        // Safe uniform mapping of ID strings to prevent matching mismatches
  ...
  const handleInlineSave = async (id: string) => {
    ...
    // 🎯 Optimistic direct single target row replacement setup
    setMovements((prev) =>
      prev.map((m) =>
        m.id === id
          ? {
